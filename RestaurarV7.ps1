...
$procedimento = $btnRestore.Add_Click({
    ...
    return "Restore" # Informa o tipo do procedimento
})

# Evento do Botao Backup
$procedimento = $btnBackup.Add_Click({
    Limpar-Botoes # Deixa a tela apenas com os campos certos
    ...
    return "Backup" # Informa o tipo do procedimento
})
...
$btnSQL.Add_Click({
    ...
    try {
        ...
        # Recebe o caminho do backup
        $caminhoBackup = $lblRetorno.Text
        # Faz o backup
        # Recebe os bancos disponiveis
        if ($instancia -eq "MSSQLSERVER") {
            sqlcmd -S $servidor -U $Usuario -P $Senha -Q "BACKUP DATABASE $nomeBanco TO $caminhoBackup"    
        } else {
            sqlcmd -S "$servidor\$instancia" -U $Usuario -P $Senha -Q "select name from sys.databases where name not in ('master','model','msdb','tempdb') order by name"
        }
        # Finaliza
        $lblRetorno.Text = "Procedimento finalizado com sucesso"
    }
    catch {
        $ErrorMessage = $_.Exception.Message # Recebe o erro
        Mostrar-Erro $ErrorMessage # Exibe a mensagem de erro
