...
$arquivoDBs = "$PastaPrincipal\TEMP\DBs.$identificacao" # Arquivo com os bancos
...
$btnDbDisponivel.Add_Click({
    ...
    $DBs = $DBs -replace " ", ''
    $DBs = $DBs -replace "--", ''
    $DBs = $DBs -replace "name", ''
    $DBs = $DBs -replace "(637rowsaffected)", ''
    $DBs.trim() -ne "" # Apaga as linhas vazias
    #$DBs = $DBs -replace "()", ''
