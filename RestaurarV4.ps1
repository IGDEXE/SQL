...
function Verificar-Instancias ([string]$servidor) {
    $servicos = Get-Service -Computer $servidor
...
$btnVerificar.Add_Click({
    try {
        ...
        if ($Online -eq "Success") {
            $instancias = Verificar-Instancias $servidor # Verifica as instancias
...
$btnDbDisponivel.Add_Click({
    #$DBs = osql -s $Servidor -u $Usuario -p $Senha -q "select name from sys.databases where name not like ('master','model','msdb','tempdb') order by name"
...
$btnSQL.Add_Click({
    ...
    try {
        Start-Sleep -s 5 # Para exemplificar o processo
